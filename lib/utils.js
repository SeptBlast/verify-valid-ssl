const { curry, allPass, anyPass, map, startsWith, endsWith, toLower, trim, pipe } = require("ramda");
...
const buildValidFormatFn = map(({ start, end }) => {
...
const isDomainMatch = curry((toCheck, base) => {
    ...
    if (!base.startsWith("*")) {
        return false;
    } // exact domain and already does not match

    base = base.slice(1);
    if (!toCheck.endsWith(base)) {
        return false;
    } // it's not a subdomain
    const domainSegment = toCheck.slice(0, -base.length);
    if (domainSegment.startsWith("-") || domainSegment.endsWith("-") || !checkDomainSegment(domainSegment)) {
        return false;
    }
    return true;
});

const isCertValidFormat = anyPass(buildValidFormatFn(CERTIFICATE_PAIRS));

const isKeyValidFormat = anyPass(buildValidFormatFn(KEY_PAIRS));
